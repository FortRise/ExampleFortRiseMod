...
public class DarkWorldRoundLogicPatch : IHookable
{
    ...
    {
        IL.TowerFall.DarkWorldRoundLogic.OnPlayerDeath += OnPlayerDeath_patch;
    }
    ...
    private static void OnPlayerDeath_patch(ILContext ctx)
    {
        ...
        cursor.EmitDelegate<Action<DarkWorldRoundLogic>>((self) => {
            ...
            Alarm.Set(entity, 90, () => {
                ...
                self.Session.CurrentLevel.ReplayViewer.Watch(self.Session.CurrentLevel.ReplayRecorder, ReplayViewer.ReplayType.Rewind, () => {
                    ...
                    self.Session.CurrentLevel.Add<DarkWorldGameOver>(new DarkWorldGameOver(self));
                });
            });
